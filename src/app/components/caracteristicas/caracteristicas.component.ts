...
import { caracteristicaItems } from './caracteristicas-items';
...
export class CaracteristicasComponent {
  ...
  caracteristicas = caracteristicaItems;
  ...
  siguiente() {
    if (this.posicionActual < this.caracteristicas.length - this.mostrarElementos) {
      this.posicionActual++;
      this.desplazarContenedor();
    } else {
      this.posicionActual = 0;
